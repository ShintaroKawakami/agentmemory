...
is_kimi_hook_root() {
  ...
  case "$normalized_hook_root" in
    */.kimi-code/hooks|*/.kimi-code/hooks/|*/.kimi/hooks|*/.kimi/hooks/) return 0 ;;
    *) return 1 ;;
  esac
}
...
emit_block_json() {
  ...
  if is_kimi_hook_root "$hook_root"; then
    PY_REASON="$reason" python3 - <<'PY'
import json
import os
...
print(json.dumps({
    "hookSpecificOutput": {
        ...
        "permissionDecisionReason": os.environ["PY_REASON"],
    }
}, ensure_ascii=False))
PY
    return 0
...
def has_bash_write(node: Any) -> bool:
    if isinstance(node, dict):
        name = tool_name(node)
        if name in {"Bash", "Shell"} and WRITE_COMMAND_RE.search(command_text(node)):
            return True
...
WRITE_TOOLS = frozenset({"Write", "Edit", "NotebookEdit", "MultiEdit", "WriteFile", "StrReplaceFile"})
...
run_quality_check_hook() {
  ...
  emit_block_json "$hook_root" "$block_reason"
  return 0
}
